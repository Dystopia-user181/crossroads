...
function updateTemp() {
	...
	tmp.isMobile = window.innerWidth < 1000 || player.hqTree;
	tmp.displayThings = []
...
function updateTempData(layerData, tmpData, isArr) {
    
    for (item in layerData){
        try {
	        if (Array.isArray(layerData[item])) {
	            updateTempData(layerData[item], tmpData[item], true)
	        }
	        else if ((!!layerData[item]) && (layerData[item].constructor === Object)) {
	            updateTempData(layerData[item], tmpData[item], false)
	        }
	        else if (isFunction(layerData[item]) && !activeFunctions.includes(item)){
	    	...
	    	console.log("Could not parse " + item + " in tmp");
	    	continue;
	    }
    }    
}
...
function constructNodeStyle(layer){
	let style = []
	if ((tmp[layer].isLayer && !(player.r.selectingEnd ? (player.r.selectedStart == layer || !tmp[player.r.selectedStart].connected.includes(layer)) && tmp[layer].row != "side" : (tmp[layer].isLayer ? !(tmp[layer].unlocked && (player[layer].unlocked || tmp[layer].canReset)) : !(tmp[layer].canClick)))) || (!tmp[layer].isLayer && tmp[layer].canClick))
