var systemComponents = {
	...
	'tree-node': {
		...
		<button v-if="nodeShown(layer)"
			v-bind:id="layer"
			v-on:click="function() {
				if (size == 'small') {showTab(layer); return}
				if (player.r.selectingStart && size != 'small') {if (player[layer].unlocked && tmp[layer].unlocked) {player.r.selectingStart = false; player.r.selectedStart = layer; player.r.selectingEnd = true;}}
				else if (player.r.selectingEnd && size != 'small') {if (player.r.selectedStart != layer && tmp[player.r.selectedStart].connected.includes(layer)) {player.r.selectingEnd = false; player.r.selectedEnd = layer;}}
				else if(tmp[layer].isLayer) {showTab(layer)}
				else {run(layers[layer].onClick, layers[layer])}
			}"

			v-bind:tooltip="(tmp[layer].tooltip == '') ? false : (tmp[layer].isLayer) ? (
				player[layer].unlocked && tmp[layer].unlocked ? (tmp[layer].tooltip ? tmp[layer].tooltip : formatWhole(player[layer].points) + ' ' + tmp[layer].resource)
			...
			v-bind:class="{
				...
				locked: player.r.selectingEnd ? (player.r.selectedStart == layer || !tmp[player.r.selectedStart].connected.includes(layer)) && size != 'small' : (tmp[layer].isLayer ? !(tmp[layer].unlocked && (player[layer].unlocked || tmp[layer].canReset)) : !(tmp[layer].canClick)),
				notify: tmp[layer].notify,
				resetNotify: tmp[layer].prestigeNotify,
				can: (((player[layer].unlocked && tmp[layer].unlocked) || tmp[layer].isLayer) && tmp[layer].isLayer) || (!tmp[layer].isLayer && tmp[layer].canClick),
			}"
	...
	'overlay-head': {
		...
		<br>
		<span class="overlayThing" style="text-shadow: 0 0 10px; font-size: 30px">Crossroads: The Incremental</span>
		<br>
		<span class="overlayThing">Current Terrain: Plains<br>Current Realm: Earthern</span>
    ...
    'info-tab': {
        ...
        <br>
        Original: The Prestige Tree made by Jacorb and Aarex (idea by papyrus)
		<br><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                ...
                <td><button class="opt" onclick="toggleOpt('hqTree')">Force Single Tab: {{ player.hqTree?"ON":"OFF" }}</button></td>
            </tr>
        </table>`
    },
